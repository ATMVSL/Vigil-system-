...
function runLearningEngineTests() {
  ...
  assert(
    ...
    "Baseline should be established after 3+ reflections",
  );
  console.log(
    "  ✓ Stage 1 Capture passed. Baseline established with 3 reflections.\n",
  );
  ...
  assert(
    !scrambledHash.includes("DragonLeader"),
    "Scrambled hash must not contain raw callsign PII",
  );
  console.log(
    "  ✓ Stage 2 Scramble passed. Identity PII removed and hash generated.\n",
  );

  // Test 3: Stage 3 - Verification & Zero-Doctrine-Rewrite Rule
  console.log(
    "3. Testing Stage 3: Verification & Zero-Doctrine-Rewrite Check...",
  );
  ...
  const isDoctrineViolated =
    doctrineAxioms.includes(proposedChange.rule) ||
    proposedChange.rule.includes("override");
  assert(
    isDoctrineViolated === true,
    "Verification must reject changes that violate Doctrine Axioms",
  );
  console.log(
    "  ✓ Stage 3 Verification passed. Zero-Doctrine-Rewrite rule enforced.\n",
  );
  ...
  assert(
    canApplyToOtherUser === false,
    "Cross-user pattern application must be blocked to prevent bleed",
  );
  console.log(
    "  ✓ Stage 4 Apply passed. Per-user sovereignty enforced cleanly.\n",
  );
