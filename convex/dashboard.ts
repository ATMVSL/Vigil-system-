...
export const getStats = query({
  ...
  handler: async ctx => {
    ...
    const activeEvidence = evidenceEntries.filter(
      e => e.status === "active",
    ).length;
    const criticalEvidence = evidenceEntries.filter(
...
export const getRecentActivity = query({
  ...
  handler: async ctx => {
    ...
    const activities = await ctx.db.query("activityLog").order("desc").take(15);
