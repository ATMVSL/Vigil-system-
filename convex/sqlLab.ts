...
export const listChallenges = query({
  ...
  handler: async (ctx, args) => {
    const q = ctx.db.query("sqlChallenges");
    if (args.category) {
      return await ctx.db
        .query("sqlChallenges")
        .withIndex("by_category", qb =>
          qb.eq("category", args.category as "select"),
        )
        .collect();
    }
...
export const submitQuery = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!passed) {
      ...
      // Check SQL structure
      const hasSelect =
        /\bselect\b/.test(userNorm) === /\bselect\b/.test(expectedNorm);
      const hasFrom =
        /\bfrom\b/.test(userNorm) === /\bfrom\b/.test(expectedNorm);
      const hasWhere =
        /\bwhere\b/.test(userNorm) === /\bwhere\b/.test(expectedNorm);
      const hasJoin =
        /\bjoin\b/.test(userNorm) === /\bjoin\b/.test(expectedNorm);
      const hasGroup =
        /\bgroup by\b/.test(userNorm) === /\bgroup by\b/.test(expectedNorm);
    ...
    if (passed) {
      ...
      const hasCorrectType =
        (userNorm.startsWith("select") && expectedNorm.startsWith("select")) ||
      ...
      if (!hasCorrectType) {
        ...
        !userNorm.includes("where") &&
        expectedNorm.includes("where")
      ) {
        ...
        !userNorm.includes("group by") &&
        expectedNorm.includes("group by")
      ) {
...
export const getStats = query({
  ...
  handler: async ctx => {
    const userId = await getAuthUserId(ctx);
    if (!userId)
      return { totalChallenges: 0, attempted: 0, passed: 0, totalPoints: 0 };
...
export const createChallenge = mutation({
  ...
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) throw new Error("Not authenticated");
    return await ctx.db.insert("sqlChallenges", {
      ...args,
      createdAt: Date.now(),
    });
  },
