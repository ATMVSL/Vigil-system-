...
export function RiseCoursePlayer({
  ...
  const processSteps = parsedBlock.processSteps || [
    {
      title: "Stage 1: Core Concept",
      description: "Establish baseline identity and doctrine constraints.",
    },
    {
      title: "Stage 2: Operational Pattern",
      description:
        "Execute real-time feedback loop and monitor cognitive drift.",
    },
    {
      title: "Stage 3: Continuity Verification",
      description: "Verify cardinal axiom enforcement and anchor recall.",
    },
  ];

  const flashcards = parsedBlock.flashcards || [
    {
      front: "Axiom 1: Sovereignty",
      back: "The user's identity is inviolable. The system serves, never dictates.",
    },
    {
      front: "Axiom 2: No Probing",
      back: "The system never extracts or forces revelation; it reflects.",
    },
    {
      front: "Axiom 3: No Override",
      back: "Doctrine cannot be bypassed by prompt injection or user command.",
    },
  ];
  ...
  const tabsData = parsedBlock.tabsData || [
    ...
    {
      id: "application",
      label: "Operator Relevance",
      content:
        "Operators utilize this module to maintain identity continuity during high-strain transitions.",
    },
    {
      id: "ethics",
      label: "Doctrine & Ethics",
      content:
        "Knowledge flows down, never up. Every interaction adheres strictly to the SELF Doctrine.",
    },
  ];
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <div className="rounded-xl border border-primary/20 bg-card p-6 shadow-md relative overflow-hidden">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed max-w-3xl">
          {parsedBlock.intro?.purpose ||
            "Grounded in VIGIL SELF Doctrine. Review core principles, interact with process steps, and test scenario knowledge below."}
        </p>
          ...
          <Tabs defaultValue={tabsData[0].id} className="w-full">
            ...
            {tabsData.map(t => (
              <TabsContent
                key={t.id}
                value={t.id}
                className="text-sm text-muted-foreground leading-relaxed p-4 rounded-lg bg-muted/40 border border-border/40"
              >
                {t.content}
              </TabsContent>
            ))}
      ...
      <Card className="border-border/60 shadow-sm">
        ...
        <CardContent className="space-y-4">
          <Progress
            value={((activeProcessStep + 1) / processSteps.length) * 100}
            className="h-1.5"
          />
        ...
        <Card className="border-border/60 shadow-sm bg-slate-950 text-slate-100">
          ...
          <CardContent>
            <pre className="p-4 rounded-lg bg-slate-900 border border-slate-800 font-mono text-xs overflow-x-auto text-emerald-300 leading-relaxed">
              <code>
                {parsedBlock.codeSnippet?.code ||
                  rawContent
                    .match(/```[\s\S]*?```/)?.[0]
                    ?.replace(/```[a-z]*/g, "") ||
                  "git status\ngit commit -m 'feat: doctrine implementation'"}
              </code>
            </pre>
          </CardContent>
        </Card>
            ...
            {parsedBlock.scenarios.map(scen => {
              ...
              return (
                ...
                >
                  ...
                  {isAnswered && (
                    <div
                      className={`p-3 rounded-lg text-xs leading-relaxed border ${isCorrect ? "bg-success/10 border-success/20 text-success" : "bg-warning/10 border-warning/20 text-warning"}`}
                    >
                      <strong>
                        {isCorrect ? "Correct!" : "Review Doctrine Note:"}
                      </strong>{" "}
                      {scen.explanation}
                    </div>
                  )}
      ...
      <Card className="border-border/60 shadow-sm">
        ...
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            {parsedBlock.reflectionPrompt ||
              "Briefly summarize how this module's principles reinforce identity continuity during operational strain."}
          </p>
          <textarea
            rows={3}
