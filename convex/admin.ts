...
// ─── DELETE USER BY CALLSIGN (Founder only) ───
export const deleteUserByCallsign = mutation({
  ...
  handler: async (ctx, { callsign }) => {
    ...
    if (!myProfile || myProfile.role !== "founder") {
      throw new Error("Only the Founder can delete users");
    }

    // Find the mirror with this callsign
    const mirrors = await ctx.db.query("mirrors").collect();
    const mirror = mirrors.find(m => m.callsign === callsign);
    if (!mirror) return { deleted: false, reason: "Mirror not found with callsign: " + callsign };

    const targetUserId = mirror.userId;

    // Don't delete founder
    const targetProfile = await ctx.db
      .query("userProfiles")
      .withIndex("by_user", (q) => q.eq("userId", targetUserId))
      .first();
    if (targetProfile?.role === "founder") {
      return { deleted: false, reason: "Cannot delete Founder" };
    }
    ...
    // Reflections
    const reflections = await ctx.db.query("reflections").withIndex("by_mirror", (q) => q.eq("mirrorId", mirror._id)).collect();
    for (const r of reflections) await ctx.db.delete(r._id);

    // Profile
    if (targetProfile) await ctx.db.delete(targetProfile._id);

    // Enrollments
    const enrollments = await ctx.db.query("enrollments").withIndex("by_user", (q) => q.eq("userId", targetUserId)).collect();
    for (const e of enrollments) await ctx.db.delete(e._id);

    // Activity log entries
    const activities = await ctx.db.query("activityLog").withIndex("by_user", (q) => q.eq("userId", targetUserId)).collect();
    for (const a of activities) await ctx.db.delete(a._id);

    // Evidence entries (linked by mirror)
    const evidence = await ctx.db.query("evidenceEntries").collect();
    for (const e of evidence) { if (e.linkedMirrorId === mirror._id) await ctx.db.delete(e._id); }

    // SQL submissions
    const sqlSubs = await ctx.db.query("sqlSubmissions").collect();
    for (const s of sqlSubs) { if (s.userId === targetUserId) await ctx.db.delete(s._id); }

    // Infra attempts
    const infraAttempts = await ctx.db.query("infraAttempts").collect();
    for (const a of infraAttempts) { if (a.userId === targetUserId) await ctx.db.delete(a._id); }

    // Exam attempts
    const examAttempts = await ctx.db.query("examAttempts").collect();
    for (const a of examAttempts) { if (a.userId === targetUserId) await ctx.db.delete(a._id); }

    // Certifications
    const certs = await ctx.db.query("certifications").collect();
    for (const c of certs) { if (c.userId === targetUserId) await ctx.db.delete(c._id); }

    // Community posts
    const posts = await ctx.db.query("communityPosts").collect();
    for (const p of posts) { if (p.userId === targetUserId) await ctx.db.delete(p._id); }

    // Delete the user record itself
    await ctx.db.delete(targetUserId);

    return { deleted: true, callsign, userId: targetUserId };
  },
});
